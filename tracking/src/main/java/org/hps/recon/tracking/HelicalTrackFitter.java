 ...
 * 7/6/2019 -- allow HPS-style 2D hits, where we allow single strip hits to help 
 * with the linear fit, but not the circle hits.  
 * @author Matt Graham
 * @version HPS-1.0 
 */
public class HelicalTrackFitter extends org.lcsim.fit.helicaltrack.HelicalTrackFitter{
    ...
    public FitStatus fit(List<HelicalTrackHit> hitcol, Map<HelicalTrackHit, MultipleScatter> msmap, HelicalTrackFit oldhelix) {
        ...
        List<HelicalTrackHit> line_hits = new ArrayList<HelicalTrackHit>();
        ...
        for (HelicalTrackHit hit : hitcol) {
            ...
            //  line hits
            if (hit instanceof HelicalTrack3DHit) line_hits.add(hit);
            if (hit instanceof HelicalTrack2DHit) line_hits.add(hit);
        ...
        if (npix > 1) {
            ...
            for (int i = 0; i < npix; i++) {
                HelicalTrackHit hit = line_hits.get(i);
                z[i] = hit.z();
     ...
     * charged particle traveling in the wrong direction (e.g. clockwise
     ...
     * particle traveling in the right direction, return the original fit.
